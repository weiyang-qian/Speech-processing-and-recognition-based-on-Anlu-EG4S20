...
set rc [catch {
  ...
  elaborate -top {audio_speak}
  export_db {ES8388_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../ES8388.adc
  optimize_rtl
  report_area -file ES8388_rtl.area
  export_db {ES8388_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea ES8388_gate.area
  legalize_phy_inst
  export_db {ES8388_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
