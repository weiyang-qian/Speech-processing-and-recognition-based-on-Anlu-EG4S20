...
set rc [catch {
  open_project {ES8388.prj}
  import_device eagle_s20.db -package EG4S20BG256
  import_db {../syn_1/ES8388_gate.db}
  place
  export_db {ES8388_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file ES8388_phy.area
  export_db {ES8388_pr.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid ES8388_inst.bid
  bitgen -bit "ES8388.bit" -unused_io_status pulldown
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
